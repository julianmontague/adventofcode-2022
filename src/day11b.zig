//! Advent of Code 2022 Day 11 part 2 solution
...
pub fn main() !void {
    ...
    var supermodulus: u64 = 1;
    for (monkeys) |monkey| {
        supermodulus *= monkey.the_test.divisible;
    }
    ...
    for (0..ROUNDS) |_| {
        for (monkeys, 0..) |monkey, i| {
            while (monkeys[i].items.items.len > 0) {
                ...
                var thrown: u64 = monkeys[i].items.orderedRemove(0);
                if (monkey.op_old) {
                    if (monkey.op == '*') {
                        thrown *= thrown;
                    } else if (monkey.op == '+') {
                ...
                thrown %= supermodulus;
                try monkeys[throw_to].items.append(@intCast(thrown));
            }
        }
    }
