...
pub fn main() !void {
    ...
    while (output != null) {
        ...
        if (fullyContained(range1Start, range1End, range2Start, range2End)) {
            sum += 1;
        }
    ...
    try out.print("Answer: {d} contained ranges\n", .{sum});
}
...
pub fn fullyContained(s1: u8, e1: u8, s2: u8, e2: u8) bool {
    if (s1 <= s2 and e1 >= e2) {
        return true;
    } else if (s1 >= s2 and e1 <= e2) {
        return true;
    } else {
        return false;
    }
